...
<div class="tw-layout">
	   ...
	   <div class="tw-list-top">
	    		...
	    		<a class="layui-btn layui-btn-normal" href="<?php echo U('add',array('pid'=>I('get.pid',0)));?>">
	                <i class="tw-icon-plus-circle"></i> 添加
	            </a>
	    		<a class="layui-btn layui-btn-danger" onclick="javascript:recycle('chkbId', '确认删除?!此步骤无法恢复!',true)">
	                <i class="tw-icon-minus-circle"></i> 删除
	            </a>
	            <a class="layui-btn" href="<?php echo U('sort',array('pid'=>I('get.pid',0)),'');?>" >
	                <i class="tw-icon-cogs"></i> 排序
	            </a>
	          </div>
	        <form action="/index.php/Admin/Menu/index/pid/16" method="get" id='frmSearch'>
	            ...
	            <table class="tw-table tw-table-list tw-table-fixed">
	                ...
	                <tbody>
	                	<?php if(!empty($list)): if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$menu): $mod = ($i % 2 );++$i;?><tr>
			                        ...
			                        <td>
				                        <a class="layui-btn" href="<?php echo U('index?pid='.$menu['id']);?>">
							                <i></i> 子菜单
							            </a>
			                        
				                        <a class="layui-btn layui-btn-normal" href="<?php echo U('edit?id='.$menu['id']);?>">
							                <i></i> 修改
							            </a>
							            <a class="layui-btn layui-btn-danger" onclick="javascript:recycle(<?php echo ($menu['id']); ?>, '确认删除?!此步骤无法恢复!',true)">
